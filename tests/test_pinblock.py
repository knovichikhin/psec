...
@pytest.mark.parametrize(
    ["pin_block", "error"],
    [
        ("041234FFFFFFFFFF", "PIN block is not ISO format 2: control field `0`"),
        ("29123456789FFFF", "PIN block must be 16 hexchars long"),
        ("29123456789FFFFX", "PIN block must be 16 hexchars long"),
        ("2X1234567890FFFF", "PIN block must be 16 hexchars long"),
        ("2F123456789012FF", "PIN length must be between 4 and 12: `15`"),
        ("22123456789012FF", "PIN length must be between 4 and 12: `2`"),
        ("2C12345678901212", "PIN block filler is incorrect: `12`"),
        ("2C12345678901FFF", "PIN is not numeric: `12345678901F`"),
...
# fmt: on
def test_decode_pin_block_iso9564_2_exception(
    pin_block: Union[bytes, str], error: str
) -> None:
    with pytest.raises(
        ValueError,
        match=error,
...
def test_decode_pin_block_iso9564_2(pin: str, pin_block: Union[bytes, str]) -> None:
