...
# deterministic
@pytest.mark.parametrize(
    ["pin", "det_pad", "pin_field"],
    [
        ("1234", "548ED7FD65495950", "441234AAAAAAAAAA548ED7FD65495950"),
    ],
)
def test_encode_pin_field_iso_4(pin: str, det_pad: str, pin_field: str) -> None:
    det_pad_bytes = bytes.fromhex(det_pad)
    assert (
        pin_field
        == pinblock.encode_pin_field_iso_4(pin, det_pad_bytes).hex().upper()
    )
...
def test_encode_pan_field_iso_4(pan: str, pan_field: str) -> None:
    assert (
        pan_field
        == pinblock.encode_pan_field_iso_4(pan).hex().upper()
    )


# fmt: off
@pytest.mark.parametrize(
    ["key", "pin", "pan", "det_pad", "error"],
    [
        (b"1" * 16, "123", "55555555555555", None, "PIN must be between 4 and 12 digits long"),
        (b"1" * 16, "1234567890123", "55555555555555", None, "PIN must be between 4 and 12 digits long"),
        (b"1" * 16, "123A", "55555555555555", None, "PIN must be between 4 and 12 digits long"),
        (b"1" * 16, "1234", "55555555555555", b"1" * 9, "Padding must be 8 bytes long."),
        (b"1" * 16, "1234", "", None, "PAN must be between 1 and 19 digits long."),
        (b"1" * 16, "1234", "555555555555555555555", None, "PAN must be between 1 and 19 digits long."),
        (b"1" * 16, "1234", "5555555555555A", None, "PAN must be between 1 and 19 digits long."),
    ],
)
# fmt: on
def test_encipher_pinblock_iso_4_exception(key: bytes, pin: str, pan: str, det_pad: bytes, error: str) -> None:
    with pytest.raises(
        ValueError,
        match=error,
    ):
        pinblock.encipher_pinblock_iso_4(key, pin, pan, det_pad)


# deterministic
@pytest.mark.parametrize(
    ["key", "pin", "pan", "det_pad", "pin_block"],
    [
        ...
        (bytes.fromhex("C1D0F8FB4958670DBA40AB1F3752EF0D"),
         "1234",
         "432198765432109870",
         bytes.fromhex("146C6601F4A8035C"),
         "CC17F65586BFD0953010226C4FC5B3CA"),
    ],
)
def test_encipher_pinblock_iso_4_det(key: bytes, pin: str, pan: str, det_pad: bytes, pin_block: str) -> None:
    assert (
        pin_block
        == pinblock.encipher_pinblock_iso_4(key, pin, pan, det_pad).hex().upper()
    )
...
def test_decode_pinblock_iso_4(pin_field: bytes, pin) -> None:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (bytes.fromhex("C1D0F8FB4958670DBA40AB1F3752EF0D"),
         bytes.fromhex("CC17F65586BFD0953010226C4FC5B3CA"),
        "432198765432109870", "1234"),
    ],
)
def test_decipher_pinblock_iso_4(key: bytes, pin_block: bytes, pan: str, pin: str) -> None:
